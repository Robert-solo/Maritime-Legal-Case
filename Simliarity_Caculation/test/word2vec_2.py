...
def model_train(train_file_name, save_model_file):  # model_file_name为训练语料的路径,save_model为保存模型名
    ...
    model = gensim.models.Word2Vec(sentences, size=200,min_count=1, workers=multiprocessing.cpu_count(),iter=5)  # 训练skip-gram模型; 默认window=5
...
cut_txt('D:\\python\\dengna_src\\law.txt')  # 须注意文件必须先另存为utf-8编码格式
 
save_model_name = 'D:\\python\\dengna_src\\law.model'
...
print(word1,"  和  ", word2, "相似度为：", y1)
print("-------------------------------\n")
 
# 计算某个词的相关词列表
y2 = model_1.most_similar(word2, topn=20)  # top n个最相关的
